...
import { buscarDadosHome } from '../services/pageData';
import { formatarNumero } from '../services/textFormatter';
    ...
    <View style={styles.container}>
      <View style={styles.row}>
        <View style={styles.col}>
          ...
          <Text style={styles.info}>{formatarNumero(data.totalVendas)}</Text>
        </View>
      </View>
      <View style={styles.row}>
        <View style={styles.col}>
          <Text style={[styles.titulo, { backgroundColor: '#237e4dff' }]}>Qtde Vendas</Text>
          <Text style={styles.info}>{formatarNumero(data.quantidadeVendas)}</Text>
        </View>
        <View style={styles.col}>
          <Text style={[styles.titulo, { backgroundColor: '#226a44ff' }]}>Ticket Médio</Text>
          <Text style={styles.info}>{formatarNumero(data.ticketMedio)}</Text>
        </View>
        <View style={styles.col}>
          <Text style={[styles.titulo, { backgroundColor: '#b6493fff' }]}>T. Cancelado</Text>
          <Text style={styles.info}>{formatarNumero(data.totalCancelado)}</Text>
        </View>
      </View>
      <View style={[
        styles.row,
        { backgroundColor: '#e9e5e5ff', borderRadius: 5, marginTop: 3 }
      ]}>
        <View style={[styles.col]}>
          <PieChart
            data={dataChart}
        ...
        <View style={[styles.col, { display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }]}>
          {dataChart.map((item, index) => (
            <View key={index} style={styles.legendaItem}>
              <View style={[styles.legendaCor, { backgroundColor: item.color }]} />
              <Text style={styles.legendaTexto}>
                {item.name}: {formatarNumero(item.population)}
              </Text>
            </View>
          ))}
        </View>
      ...
      <View>
        ...
        {data.produtosMaisVendidos.map((item: any, index: number) => (
          <View key={index} style={
            { backgroundColor: '#e9e5e5ff', padding: 5, borderBottomWidth: 1, borderBottomColor: '#c1c1c1' }
          }>
